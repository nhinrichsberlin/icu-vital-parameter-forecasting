...
class TSTransformerEncoderClassiRegressor(nn.Module):
    ...
    def __init__(self,
        ...
        pos_encoder = _get_pos_encoder(pos_encoding)
        self.pos_enc = pos_encoder(d_model=d_model,
                                   dropout=dropout * (1.0 - freeze),
                                   max_len=max_len)

        encoder_layer = _get_encoder_layer(norm=norm,
                                           d_model=d_model,
                                           n_heads=n_heads,
                                           dim_feedforward=dim_feedforward,
                                           dropout=dropout * (1.0 - freeze),
        ...
        self.output_layer = nn.Linear(d_model, num_classes * horizon)
    ...
    def forward(self, X, padding_masks=None):
        ...
        Returns:
            output: (batch_size, seq_length, horizon, num_classes)
        """

        if padding_masks is None:
            padding_masks = torch.ones(X.shape[0], X.shape[1], dtype=bool).to(X.device)

        # mask future steps
        future_mask = nn.Transformer.generate_square_subsequent_mask(X.size(1)).to(X.device)
